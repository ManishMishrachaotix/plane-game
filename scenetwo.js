...
export class SceneTwo extends Phaser.Scene {
  ...
  disappearCardsAndTalon(scene) {
    const allCards = [
      ...
      this.cardJ, this.cardJBack, cardQ, cardQBack, this.cardK
    ].filter(Boolean);
    scene.tweens.add({
  ...
  create() {
    try {
      ...
      this.tweens.add({
        ...
        onComplete: () => {
          this.tweens.add({
            targets: [cardA, cardABack, card8, card8Back, card9, card9Back, card10, card10Back, this.cardK],
            y: (target) => {
              ...
              if (target === this.cardK) return 1700;
              return target.y;
            },
            duration: 300,
            ease: 'Power1',
            onComplete: () => {
              ...
              this.tweens.add({
                targets: [card8, card8Back, this.cardK],
                x: (target) => {
                  if (target === card8 || target === card8Back) return 390;
                  if (target === this.cardK) return 690;
                  return target.x;
                },
                duration: 300,
                ease: 'Power1',
                onComplete: () => {
                  ...
                  console.log('Card K position:', { x: this.cardK.x, y: this.cardK.y });
                  enableCardTap(this);
